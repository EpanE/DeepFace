...
# Dictionary of model names and their embedding dimensions
MODEL_DIMENSIONS = {
    'Facenet': 128,
    'Facenet512': 512,
    'OpenFace': 128,
...
class LiveFaceRecognitionSystem:
    def __init__(self,
        ...
        # Generate model-specific embedding filename
        self.embedding_file = f'face_embeddings_{model.lower()}.pkl'
        ...
        if not os.path.exists(attendance_log_path):
            with open(attendance_log_path, 'w', newline='') as csvfile:
                ...
                writer.writerow(['Name', 'Time', 'Process Time (ms)', 'Model'])

    def load_or_create_embeddings(self):
        """
        Load existing embeddings if compatible with current model, otherwise create new ones
        """
        try:
            if os.path.exists(self.embedding_file):
                ...
                if self.known_embeddings:
                    sample_embedding = next(iter(self.known_embeddings.values()))
                    if len(sample_embedding) == MODEL_DIMENSIONS.get(model, 0):
                        print(f"Embeddings loaded successfully for model {model}")
                        return
                    else:
                        print(f"Existing embeddings incompatible with model {model}")

            # If we reach here, either file doesn't exist or embeddings are incompatible
            print(f"Creating new embeddings for model {model}")
            self.known_embeddings = self.compile_face_embeddings()
    ...
    def compile_face_embeddings(self):
        """
        Compile face embeddings from database directory into a pickle file
        """
        known_embeddings = {}
            ...
            if os.path.isdir(folder_path):
                try:
                    ...
                    if image_files:
                        ...
                        print(f"Created embedding for {person_folder}")
                except Exception as e:
                    print(f"Error creating embedding for {person_folder}: {e}")
    ...
    def create_new_face_directory(self, name):
        ...
        if ret:
            ...
            # Recompile embeddings for current model
            self.known_embeddings = self.compile_face_embeddings()
        else:
    ...
    def run_live_recognition(self, video_source=0):
        ...
        # Model display
        ttk.Label(info_frame, text="Model:", style='Small.TLabel').grid(row=6, pady=2)
        model_display = ttk.Label(info_frame, text=model, style='SmallBold.TLabel')
        model_display.grid(row=7, pady=0)

        # Buttons
        button_frame = ttk.Frame(info_frame)
        button_frame.grid(row=8, pady=5)
